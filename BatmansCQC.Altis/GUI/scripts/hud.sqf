...
switch _event do {
    ...
    case "hudDraw3D": {
        ...
        private _ctrlTimerGroup = _display displayCtrl IDC_HUD_TIMERGROUP;

        if (CQC_timerTime > time) then {
            if !(ctrlShown _ctrlTimerGroup) then { 
                _ctrlTimerGroup ctrlShow true;
            };
            
            private _timer = IDC_HUD_TIMERGROUP controlsGroupCtrl IDC_HUD_TIMERTIME;
            
            _timer ctrlSetText [CQC_timerTime - time] call BIS_fnc_secondsToString;
        } else {
            if (ctrlShown _ctrlTimerGroup) then { 
                _ctrlTimerGroup ctrlShow false;
            };
        };
    };
