...
// Admin
if !(isNil {call (missionNamespace getVariable "CQC_player_adminLevel")}) exitWith {endMission "CQC_InitError"};
...

{
    ...
    if (_id == getPlayerUID player) then {
        [missionNamespace, "CQC_player_adminLevel", _rank] call CQC_fnc_setConstant;
        [player, "CQC_player_alias", _alias, true] call CQC_fnc_setConstant;
        breakOut "admin";
    } else {
        if (count _admins == (_forEachIndex + 1)) then {
            [missionNamespace, "CQC_player_adminLevel", 0] call CQC_fnc_setConstant;
            [player, "CQC_player_alias", "", true] call CQC_fnc_setConstant;
        };
    };
} forEach _admins;
