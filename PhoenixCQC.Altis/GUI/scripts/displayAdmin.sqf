...
switch _event do {
    case "onLoad": {
		...
		if !(call CQC_player_adminLevel > 0) exitWith {_display closeDisplay 1; closeDialog 0;};

		// Unload HUD Elements
		showChat false;
		"hide" call CQC_fnc_showHUD;
        ...
        _ctrlButtonDrawEntities ctrlEnable (call CQC_player_adminLevel > 2);
        ...
        _ctrlButtonArsenal ctrlEnable (call CQC_player_adminLevel > 1);
        ...
        _ctrlButtonDebug ctrlEnable (call CQC_player_adminLevel > 2);
		...
		_ctrlButtonEject ctrlShow (call CQC_player_adminLevel > 2);
		...
		_ctrlButtonSpeed ctrlShow (call CQC_player_adminLevel > 2);
		_ctrlButtonSpeed ctrlEnable false;*/

		private _ctrlSeperator7 = _display displayCtrl IDC_DISPLAYADMIN_SEPERATOR_7;
		_ctrlSeperator7 ctrlShow (call CQC_player_adminLevel > 2);
    ...
    case "updatePlayerList" : {
		...
		// Go through groups and get the valid ones only, also cache group units information
		{
			...
			private _groupTexture = ["getGroupTextures", [_group]] call SELF;
			...
			// Validate units
			{
				if (isPlayer _x) then
				{
					_unitsInfo pushBack [
						...
						[_x, true, 17] call BIS_fnc_getName, 
						_group, 
						vehicle _x, 
...

					{
						...
						if (_i isEqualTo []) then
						{
							...
							_ctrl tvSetPictureRight [[_sideIndex, _groupIndex, _unitIndex], switch (true) do {
								...
								case (_unit getVariable ["adminOpen", false]): 	{"a3\modules_f_curator\data\portraitcurator_ca.paa"};
	...
	case "setFocus": {
		...
		if (!isNull _newFocus) then {
			...
			(_display displayCtrl IDC_DISPLAYADMIN_STATUSBAR_LINKGAMECP) ctrlSetStructuredText parseText format ["<t size='0.775'><a href='https://grandtheftarma.com/gamecp/player/%1'>%1</a></t>", getPlayerUID _newFocus];
	...
	case "togglePannelLeftOn": {
		...
		if (call CQC_player_adminLevel < 3) then {
			{
				_x ctrlShow false;
	...
	case "teleportToCursor": {
		...
		if (call CQC_player_adminLevel > 1) then {
			private _focus = uiNamespace getVariable ["CQC_displayAdmin_focus", objNull];
